...
WIM.AddLocale("ruRU", {
    ...
    ["Hides these messages from the game's chat frame so they appear only in WIM. Per-type and per-channel 'Never Suppress' exceptions on the Conversations pages win. A message is not hidden when no WIM window would show it."] = "Скрывает эти сообщения из окна чата игры, чтобы они отображались только в WIM. Исключения 'Никогда не скрывать' для типов и каналов на страницах раздела Беседы имеют приоритет. Сообщение не скрывается, если ни одно окно WIM его не покажет.", -- Google Translate placeholder until native language speaker can confirm locale or suggest corrected translation
